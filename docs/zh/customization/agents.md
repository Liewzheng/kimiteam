---
url: /kimiteam/docs/zh/customization/agents.md
---
# Agent 与子 Agent
...
|`${now}`| 当前时间（ISO 格式） |
|`${additional\_dirs\_info}`| 加入工作区的额外目录信息；没有时为空 |
|`${base\_prompt}`| 默认系统提示词。在`SYSTEM.md`中指内置默认提示词；在 Agent 文件中指有效默认提示词（内置默认，或存在时为你的`SYSTEM.md`覆盖） |
|`${plugin\_sections}\` | 已启用 plugin 提供的完整 Plugin Instructions 块；没有已启用 plugin 提供指令时为空 |
