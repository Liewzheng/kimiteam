---
url: /kimiteam/docs/zh/configuration/env-vars.md
---
# 环境变量
...
| `KIMI_CODE_AGENT_SWARM_MAX_CONCURRENCY` | 子 Agent 并发上限（session 级），优先级最高（env > 运行时 `TeamConcurrency` > config `max_concurrency`）；不设置表示不限制 | 正整数；非法值被忽略并记录警告，回退到下一优先级 |
