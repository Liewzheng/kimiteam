---
url: /kimiteam/docs/zh/configuration/config-files.md
---
# 配置文件
...
::: tip
TOML 字段名一律用下划线（snake\_case），如 `default_model`、`max_context_size`。字段名里若含 `.`，需用引号包住，例如 `[models."gpt-4.1"]`——否则 TOML 会把 `.` 解释为嵌套表分隔符。
:::
...
```toml
# chat-level overwrite: coder 成员不再使用 secondary 模型而固定用 kimi-for-coding
