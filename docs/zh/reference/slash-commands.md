---
url: /kimiteam/docs/zh/reference/slash-commands.md
---
# 斜杠命令
...
| `/team [on\|off]` | — | 不带参数时打开团队总览面板（全员职位、生效模型、平均分/评分次数）；传 `on` 开启团队模式（使五个管理工具对主 Agent 可见），传 `off` 关闭。终端子 Agent 调用卡片会显示"名字 · 职位 · 模型 · 上次评分"。详见[组建你的团队](../guides/team.md) | 是 |
