---
url: /kimiteam/docs/zh/guides/team.md
---
# 组建你的团队
...
* **后台默认**：团队模式下，主 Agent 派工默认后台运行——`Agent` 工具的 `run_in_background` 省略即视为 `true`。派工立即返回 task\_id，完成结果以系统消息自动送达，主 Agent 不阻塞，随时可与用户沟通。只有显式要求前台等待，才会同步阻塞。
