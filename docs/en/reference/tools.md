---
url: /kimiteam/docs/en/reference/tools.md
---
# Built-in Tools
...
## Team Management (Main Agent Only)

These four tools are available to the main Agent alone; subagents calling them will be rejected.

| Tool | Default Approval | Description |
| --- | --- | --- |
| `TeamHire` | Requires approval | Hire a new team member: creates an agent profile in user or project directory under `.kimi-code/agents`; errors if same-name exists |
| `TeamFire` | Requires approval | Fire a member: deletes the corresponding agent file; performance data is retained |
| `TeamScore` | Requires approval | Score work: 0–100 with notes, attributed to the running model id |
| `TeamMessage` | Requires approval | Deliver a message: soft reminder (insert into current turn) or interrupt-and-correct (cancel current turn first) |

**`TeamHire`** accepts parameters `name` (kebab-case file name), `role` (optional job title), `description`, `whenToUse` (optional), `prompt` (system prompt body), `model` (optional — any model id from `[models.*]` or `"primary"` / `"secondary"`; omitted means the profile's `model_preference` controls subsequent behavior), `tools` / `disallowedTools` (optional, same syntax as agent file), `subagents` (optional), `duty` (optional boolean), and `scope` (optional `"user"` or `"project"`, default `"user"`). Writes to `~/.kimi-code/agents/<name>.md` or `<cwd>/.kimi-code/agents/<name>.md`. If same-name already exists, errors instead of silently overwriting; prompt users to fire then hire. After writing, the directory watcher auto-reloads and the member is immediately dispatchable. When autonomously forming a team, the main Agent should directly decide name / role / description / prompt / model without asking item by item — available model ids are listed in `[models]` config section.

**`TeamFire`** accepts `name` and optional `scope` (defaults to same as Hire). Locates and deletes the corresponding agent file; errors if not found, listing all currently existing profiles. Performance records in `performance.json` are retained. The watcher auto-reloads afterwards, making that member undispachable.

**`TeamScore`** accepts `profile` (agent name to score), `score` (integer 0–100), `note` (review comments), and optional `model` (omitted — tool auto-resolves the profile's current effective binding). Writes to persistent storage and echoes the profile's updated summary (with byModel grouping results). The core purpose of scoring is to evaluate **whether a "member × model" pairing is competent** — when a profile consistently scores low on a particular model, the main Agent should switch via `[subagent.model_overrides]` or recommend another available id from `[models]`.

**`TeamMessage`** accepts `agent_id` (task ID returned by Agent/AgentSwarm dispatch results and TaskOutput metadata), `message` (text to deliver), and optional `interrupt` (default false):

* **Soft reminder** (`interrupt: false`, default): injects the message as a synthetic user message into the running subagent's current turn; if idle, starts a new turn.
* **Interrupt-and-correct** (`interrupt: true`): cancels the subagent's current turn output first, then injects the new instruction — semantically equivalent to pressing ESC twice in the terminal to interrupt and re-enter.

Fire-and-forget single delivery; responses flow back through existing task result/ notification channels. If the target agent does not exist, lists available agents; for terminated members, suggests `Agent resume` instead.
