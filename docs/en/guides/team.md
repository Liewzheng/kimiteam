...
Each score entry is attributed to the model id actually running at scoring time, persisted to `$KIMI_CODE_HOME/agents/performance.json` — grouped by profile name, each entry carrying timestamp, score, note, and the running model id. When a member consistently scores low on a certain model, it signals a poor "member × model" pairing; the main Agent should switch via `[subagent.model_overrides]` or recommend another available id from your `[models]` list.
