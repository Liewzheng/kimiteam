---
url: /kimiteam/docs/en/configuration/env-vars.md
---
...
| `KIMI_CODE_AGENT_SWARM_MAX_CONCURRENCY` | Session-level subagent concurrency cap; highest priority (env > runtime `TeamConcurrency` > config `max_concurrency`); leave unset for no cap | Positive integer; invalid values are ignored with a warning, falling back to the next priority level |
