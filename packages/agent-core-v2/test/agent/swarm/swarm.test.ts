...
function stubAgentTasks(): IAgentTaskService {
  return {
    ...
    registerTask: vi.fn(() => 'agent-00000000'),
  } as unknown as IAgentTaskService;
}

function stubToolPolicy(active = false): IAgentToolPolicyService {
  return {
    _serviceBrand: undefined,
    isToolActive: () => active,
...
describe('AgentSwarmTool', () => {
  it('applies one subagent_type across templated subagents', async () => {
    ...
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), swarmMode, stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubAgentTasks(), stubToolPolicy());
    const input = {
  ...
  it('does not expose permission rule argument matching', () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubAgentTasks(), stubToolPolicy());
  ...
  it('passes the configured subagent timeout to swarm tasks', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ timeoutMs: 5_000 }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubAgentTasks(), stubToolPolicy());
  ...
  it('resolves spawn task bindings from the configured secondary model', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ model: 'provider/secondary', defaultEffort: 'low' }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' }), stubAgentTasks(), stubToolPolicy());
  ...
  it('advertises both selectable models in the description only when configured', async () => {
    const host = mockSwarmHost();
    const configured = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ model: 'provider/secondary' }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model' }), stubAgentTasks(), stubToolPolicy());
    ...
    const unconfigured = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model' }), stubAgentTasks(), stubToolPolicy());
