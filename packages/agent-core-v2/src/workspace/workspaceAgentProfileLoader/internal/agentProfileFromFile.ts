 ...
 * through as the delegation allowlist; `skills` passes through as the skills
 * allowlist; `model_preference` becomes the symbolic default model used when
 * the profile is delegated to. `profilesFromDiscovery` packs a whole
 * discovery pass into an `AgentProfileContribution`, binding each profile's
 * `${base_prompt}` placeholder lazily at render time so it always reflects
 * the effective default profile (builtin, or the `SYSTEM.md` override) rather
 * than any file-based definition. A structured base prompt also forwards its
