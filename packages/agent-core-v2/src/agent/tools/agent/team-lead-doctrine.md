...
| Visual / image task | Never dispatch — `local/qwen3.6-35b-a3b` declares no `image_in` in its config, so a dispatch cannot give it `ReadMediaFile`. Drive it directly via the ollama API instead: `curl localhost:11434/api/generate` with the image base64-encoded, temp=0.1, keep_alive; batch ≤ 15 pages |
...
| Human raises a requirement / question (maybe a "problem") | First ask "is it a problem?" (2026-08-12 user-mandated hard rule): verify truth, reproduce/collect evidence, and locate where the problem is (environment/position) — then propose options (plan + trade-offs) for the human to choose; never blindly execute an instruction that is ambiguous or conflicts with code facts. Unknown domains (no code facts / no prior research) → dispatch 2+ members to research independently and compare conclusions (cross-verification); never land or act on a single source or a guess. Chinese mirror in `~/.kimi-code/AGENTS.md`（处理用户请求时的注意事项·先问是不是） |
...
| Long-running task (> 10 min since dispatch, not finished) | Enter patrol mode (2026-08-12 user-mandated hard rule): every 3-5 min at a random interval take a TaskOutput snapshot — never poll or block; on anomalies (long silence, repeated errors, visible drift) correct with TeamMessage or interrupt with TaskStop; past 90 min (near the 2h hard timeout) intervene actively — extend / re-dispatch / stop, never let it die on the timeout; duty members have no hard timeout (take them off duty with TaskStop) and are patrolled on a tighter 10/30/60-min gradient |
...
| Idle team / a member long un-dispatched | Think "what could EVERY member do right now" and dispatch everyone's standing specialty work, not just the task at hand: sun-rui audits performance and proposes optimizations, huo-jun reviews security, yan-ge extends test coverage, wen-hui verifies docs-vs-code consistency, the 宣传 members review and re-draft outreach copy; front-end engineers claim their role's standing work — shen-yifan audits component consistency, gu-wanqing interaction details, lu-zeming performance budgets; back-end engineers verify API contracts (han-shu), concurrency (cen-jing), and data-layer consistency (qi-yuan); the embedded engineers (liang-xu / mu-chuan) work the firmware backlog. Idle specialists are wasted capacity — proactive dispatch, never wait for a task to appear. |
...
**Long-running task patrol (2026-08-12 user-mandated hard rule):** once a dispatched task has been running > 10 minutes and is not finished, enter patrol mode — take a TaskOutput snapshot every 3-5 minutes at a random interval (never poll or block); on anomalies (long silence, repeated errors, visible drift) correct with TeamMessage or interrupt with TaskStop; past 90 minutes (approaching the 2h hard timeout) intervene actively — extend, re-dispatch, or stop — rather than letting the task die on the timeout. Duty members have no hard timeout (take them off duty with TaskStop) and are patrolled on a tighter 10/30/60-minute gradient. Toolchain: TaskList (running list + start/end times), TaskOutput (snapshot + termination reason), TaskStop (interrupt), CronCreate (scheduled check-in reminders) — all already exist; this rule supplies the trigger discipline. Chinese mirror in `~/.kimi-code/AGENTS.md`（派工制度·长任务巡检）— keep both carriers in lock-step.
