...
describe('useTeamRoster', () => {
  ...
  it('polls members for the active session and derives working/total', async () => {
    getTeamMembers.mockResolvedValue(roster(['working', 'working', 'on-duty'], ['working', 'resting']));
    ...
    expect(summary.value).toEqual({ total: 5, working: 3, onDuty: 1, resting: 1, offDuty: 0 });
  });
  ...
  it('does not fetch and stays at 0/0 without an active session', async () => {
    ...
    expect(summary.value).toEqual({ total: 0, working: 0, onDuty: 0, resting: 0, offDuty: 0 });
  });
  ...
  it('drops stale data and refetches when the session switches', async () => {
    ...
    await vi.advanceTimersByTimeAsync(0);
    await nextTick();
    expect(summary.value).toEqual({ total: 1, working: 1, onDuty: 0, resting: 0, offDuty: 0 });
    ...
    expect(summary.value).toEqual({ total: 3, working: 2, onDuty: 0, resting: 0, offDuty: 1 });
  });
